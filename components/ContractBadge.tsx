...
import { ZCAT_CA, EXPLORER_BASE, DEXSCREENER_URL, TOKEN_SYMBOL } from "@/lib/tokenConfig";
...
export default function ContractBadge({ variant = "footer" }: { variant?: "hero" | "footer" }) {
  ...
  return (
    <div className={`ca-badge ${variant === "hero" ? "ca-hero" : "ca-footer"}`}>
      ...
      <a className="ca-dex" href={DEXSCREENER_URL} target="_blank" rel="noopener noreferrer" title={`DexScreener · ${t("token.chart")}`}>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img className="ca-dex-logo" src="/brand/dexscreener.png" alt="DexScreener" width={20} height={20} />
        <span className="ca-dex-txt">{t("token.chart")}</span>
        <span className="ca-live" />
      </a>
    </div>
  );
}
